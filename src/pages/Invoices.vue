  ...
  <div>
    ...
    <div class="table-container">
      ...
      >
        ...
        <DxToolbar><DxItem name="searchPanel" /></DxToolbar>
        <DxColumn
          data-field="id"
          caption="No Invoice"
          :width="120"
...
export default {
  ...
  data() {
    return { invoices, formatRupiah, statusLabels };
  },
  computed: {
    summaryCards() {
      return [
        ...
        },
        {
          ...
          value: this.invoices.filter((i) => i.status === "selesai").length,
          color: "#16A34A",
        },
      ];
    },
  ...
  methods: {
    dueDateCellRender(cellInfo) {
      ...
      return `<span style="color:${color};${isOverdue ? "font-weight:600" : ""}">${date}</span>`;
    },
    amountCellRender(cellInfo) {
      return `<span style="font-weight:600">${formatRupiah(cellInfo.data.jumlah)}</span>`;
    },
    ...
    actionCellRender() {
      return `<div style="display:flex;gap:2px;justify-content:center">
        <button class="btn btn--icon btn--ghost"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg></button>
        <button class="btn btn--icon btn--ghost"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M17 3l4 4-7 7H10v-4l7-7z"/></svg></button>
        <button class="btn btn--icon btn--ghost"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg></button>
